 ...
 * separately — minimum wage, Form I-9, PRWORA new-hire reporting, and
 * state SUI/withholding registration. Pure aggregation, the same "no new
 * business logic, just call what already exists and collect the
 * results" discipline payroll/reports.ts applies to its own headcount
 * and payroll-cost rollups: every individual finding here is produced by
 * a function this project already ships and independently tests: a
 * wrong dashboard entry would be a wrong CALL SITE, never a new wrong
 * calculation.
 *
 * Before this, an admin had to know to click into four separate panels
 * to find out whether anything needed attention at all — a genuinely
 * dangerous gap for a compliance-shaped product, where the whole point
 * is that someone forgets to go looking.
 */
...
export interface ComplianceDashboard {
  ...
  /** Sum of every finding across all four categories — a single number for a "N items need attention" banner, never a substitute for looking at the categories themselves. */
...
export function computeComplianceDashboard(
  ...
  return { asOfDate, minimumWageIssues, i9Issues, newHireReportingIssues, stateRegistrationIssues, totalIssueCount };
}
